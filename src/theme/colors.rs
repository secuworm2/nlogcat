...
// ── Static constants (dark-theme palette, used by dark_visuals/light_visuals) ─
...
/// 로그 레벨 뱃지 색상. 다크/라이트 모드에 따라 다른 색조를 반환한다.
#[must_use]
pub fn level_label_color(level: LogLevel, dark_mode: bool) -> Color32 {
    if dark_mode {
        match level {
        ...
        match level {
            LogLevel::Debug => Color32::from_rgb(2, 132, 199),
            LogLevel::Info => Color32::from_rgb(22, 163, 74),
            LogLevel::Warn => Color32::from_rgb(161, 98, 7),
            LogLevel::Error => Color32::from_rgb(220, 38, 38),
            LogLevel::Fatal => Color32::from_rgb(190, 18, 60),
            LogLevel::Verbose | LogLevel::Unknown => Color32::from_rgb(75, 85, 99),
        }
    }
}

/// 로그 레벨 행 배경색. 다크/라이트 모드에 따라 다른 색조를 반환한다.
#[must_use]
pub fn level_row_bg(level: LogLevel, dark_mode: bool) -> Color32 {
    if dark_mode {
        match level {
            LogLevel::Verbose | LogLevel::Debug | LogLevel::Unknown => {
        ...
        match level {
            LogLevel::Verbose | LogLevel::Debug | LogLevel::Unknown => Color32::TRANSPARENT,
            LogLevel::Info => Color32::from_rgba_unmultiplied(22, 163, 74, 18),
            LogLevel::Warn => Color32::from_rgba_unmultiplied(202, 138, 4, 22),
            LogLevel::Error => Color32::from_rgba_unmultiplied(220, 38, 38, 22),
            LogLevel::Fatal => Color32::from_rgba_unmultiplied(190, 18, 60, 22),
        }
    }
}
